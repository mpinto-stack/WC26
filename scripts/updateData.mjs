...
async function fetchFootballDataData(groups) {
  ...
  if (!FOOTBALL_DATA_API_KEY) return { actualGroups: fallbackGroups, actualKo: fallbackKo, slotDates: fallbackSlots, matchesCount: 0, source: 'missing-key' };
  ...
  const actualGroups = {}; const actualKo = {}; const slotDates = JSON.parse(JSON.stringify(DEFAULT_SLOT_DATES)); const koBuckets = { r32: [], r16: [], qf: [], sf: [], third: [], final: [] };
  for (const m of matches) {
    const home = normalizeTeamName(m.homeTeam?.name || ''); const away = normalizeTeamName(m.awayTeam?.name || ''); const stage = m.stage || m.group || ''; const utcDate = m.utcDate || null;
  ...
  for (const [roundKey, arr] of Object.entries(koBuckets)) {
    ...
    arr.forEach((m, i) => { slotDates.knockout[roundKey][i] = m.utcDate; if (m.played) actualKo[`${roundKey}|${i}`] = { played:true, a:m.a, b:m.b, ga:m.score.home, gb:m.score.away }; });
  }
  return { actualGroups, actualKo, slotDates, matchesCount: matches.length, source: 'football-data.org' };
}
...
async function main() {
  const groups = await readJson('groups.json'); const teams = await readJson('teams.json'); let resultsInfo; let oddsInfo;
  try { resultsInfo = await fetchFootballDataData(groups); } catch (err) { console.error('Erro nos resultados:', err.message); resultsInfo = { actualGroups: await readJson('actual_groups.json', {}), actualKo: await readJson('actual_ko.json', {}), slotDates: await readJson('slot_dates.json', DEFAULT_SLOT_DATES), matchesCount: 0, source: 'fallback-after-error', error: err.message }; }
  ...
  const meta = { generatedAt: new Date().toISOString(), version: 'v6.0', footballDataCompetition: FOOTBALL_DATA_COMPETITION, oddsSportKey: ODDS_SPORT_KEY, resultsSource: resultsInfo.source, oddsSource: oddsInfo.source, counts: { matchesReturnedByFootballData: resultsInfo.matchesCount, oddsEventsReturned: oddsInfo.oddsEvents, groupResultsWritten: Object.keys(resultsInfo.actualGroups).length, koResultsWritten: Object.keys(resultsInfo.actualKo).length, slotDatesGroupsWritten: Object.keys(resultsInfo.slotDates?.groups || {}).length, oddsTeamsWritten: Object.keys(oddsInfo.odds).length }, errors: { results: resultsInfo.error || null, odds: oddsInfo.error || null } };
  await writeJson('actual_groups.json', resultsInfo.actualGroups); await writeJson('actual_ko.json', resultsInfo.actualKo); await writeJson('slot_dates.json', resultsInfo.slotDates); await writeJson('odds.json', oddsInfo.odds); await writeJson('meta.json', meta);
