...
async function fetchFootballDataData(groups, knownTeamsSet) {
  const fallbackGroups = await readJson('actual_groups.json', {}); const fallbackKo = await readJson('actual_ko.json', {}); const fallbackSlots = await readJson('slot_dates.json', DEFAULT_SLOT_DATES); const fallbackMatchups = await readJson('slot_matchups.json', DEFAULT_SLOT_MATCHUPS);
  ...
  for (const [roundKey, arr] of Object.entries(koBuckets)) {
    ...
    arr.forEach((m, i) => { slotDates.knockout[roundKey][String(i)] = m.utcDate; if (m.a && m.b) slotMatchups.knockout[roundKey][String(i)] = { a:m.a, b:m.b, utcDate:m.utcDate }; if (m.played) actualKo[`${roundKey}|${i}`] = { played:true, a:m.a, b:m.b, ga:m.score.home, gb:m.score.away }; });
  }
...
async function main() {
  ...
  const meta = { generatedAt: new Date().toISOString(), version: 'v7.0', footballDataCompetition: FOOTBALL_DATA_COMPETITION, oddsSportKey: ODDS_SPORT_KEY, resultsSource: bracketInfo.source, oddsSource: oddsInfo.source, counts: { matchesReturnedByFootballData: bracketInfo.matchesCount, groupResultsWritten: Object.keys(bracketInfo.actualGroups).length, koResultsWritten: Object.keys(bracketInfo.actualKo).length, slotMatchupsR32Written: Object.keys(bracketInfo.slotMatchups?.knockout?.r32 || {}).length, oddsTeamsWritten: Object.keys(oddsInfo.odds).length }, errors: { results: bracketInfo.error || null, odds: oddsInfo.error || null } };
